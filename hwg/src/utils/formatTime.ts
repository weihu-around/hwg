...
import localizedFormat from 'dayjs/plugin/localizedFormat' // 引入 localizedFormat 插件
import weekday from 'dayjs/plugin/weekday' // 引入 weekday 插件
import 'dayjs/locale/zh-cn' // 引入中文支持（根据需要）
...
/**
 * 时间日期转换
 * @param date 当前时间，new Date() 格式 或 字符串
 ...
 * @returns 返回拼接后的时间字符串
 */
export function formatDate(date: Date | string | number, format?: string, week?: boolean): string {
  // 日期不存在，则返回空
  if (!date) {
    return ''
  }
  ...
  const dayOfWeek = weekDays[dayjs(date).day()] // 获取星期几，返回索引从 0 到 6
  // 日期存在，则进行格式化
  if (week) {
    return dayjs(date).format(format ?? 'YYYY-MM-DD HH:mm:ss') + ` 星期${dayOfWeek}`
  }
